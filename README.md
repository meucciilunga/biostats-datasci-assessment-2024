...
Code for Q1 scrapes Wikipedia webpage for article 'List of natural disasters by death toll' and loads up data for two seperate tables in section 'Deadliest natural disasters by year excluding epidemics and famines.' Table 1 is list of deadliest disasters in 20th century, with each entry in the table detailing the year of incident, death toll of incident, formal event name, imapcted countries, natural disaster type, and date of incident. Table 2 is nearly identical, but for disasters from 2001-2024. Tables were merged together after scraping and data sanitization, then plotted via plotly as both *.png image file and interactive HTML. Death toll values were sanitized and modified according to spec. provided in original assessment paper provider by examiners. Resulting table from scraping code is as follows:
